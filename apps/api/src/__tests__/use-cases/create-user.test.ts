...
describe('createUser use case', () => {
  it('returns Ok(UserEntity) on happy path', async () => {
    ...
    vi.mocked(passwordService.hash).mockReturnValue(okAsync('hashed-pw'))
  ...
  it('returns Err when password hash fails', async () => {
    // eslint-disable-next-line @typescript-eslint/typedef
    const repo = makeMockRepo()
    // eslint-disable-next-line @typescript-eslint/typedef
    const passwordService = makeMockPasswordService()
    ...
    vi.mocked(passwordService.hash).mockReturnValue(errAsync(hashError))
