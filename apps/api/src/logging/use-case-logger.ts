...
interface UseCaseLoggerParams<TParams, TResult, TError> {
  ...
  readonly useCase: (params: TParams) => ResultAsync<TResult, TError>
  readonly getEntityId?: (result: TResult) => string | undefined
...
const withAuditLog: <TParams, TResult, TError>(
...
) => (useCaseParams: TParams & AuditableParams) => ResultAsync<TResult, TError> = (params) => {
  ...
  return (useCaseParams) => {
    ...
    return useCase(useCaseParams)
      .map((result) => {
        const entityId: string | undefined = getEntityId ? getEntityId(result) : undefined
