...
describe("createPassword", () => {
  it(
    "returns Ok(Password) for valid password with 8+ chars",
    () => {
  ...
  it(
    "returns Err(InvalidPassword) when too short",
    () => {
      // eslint-disable-next-line @typescript-eslint/typedef
  ...
  it(
    "returns Err(WeakPassword) when no digit present",
    () => {
      // eslint-disable-next-line @typescript-eslint/typedef
      const result = createPassword({
        value: "Abcdefgh",
  ...
  it(
    "returns Err(WeakPassword) when no letter present",
    () => {
      // eslint-disable-next-line @typescript-eslint/typedef
      const result = createPassword({
        value: "12345678",
  ...
  it(
    "returns Err(InvalidPassword) for empty string",
    () => {
      // eslint-disable-next-line @typescript-eslint/typedef
